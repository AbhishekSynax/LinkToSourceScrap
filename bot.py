#!/usr/bin/env python3
"""
ULTIMATE WEBSITE DOWNLOADER BOT - SYNAX EDITION (Enhanced & Fixed)
Combined Features:
...
async def handle_bulk_key_form(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle bulk key generation form"""
    query = update.callback_query
    user_id = update.effective_user.id
...
async def show_users_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Show users list - SYNAX System (Enhanced)"""
    query = update.callback_query
    user_id = update.effective_user.id
    ...
    for uid_str, user_data in sorted_users[:20]:
        count += 1
        username = user_data.get('username', 'N/A')
        first_name = user_data.get('first_name', '')
        downloads = user_data.get('total_downloads', 0)
        last_active = datetime.fromisoformat(user_data['last_active']).strftime('%d/%m')
        status = "🚫" if user_data.get('is_banned', False) else "✅"
        
        users_text += f"{count}. {status} `{uid_str}`\n"
        users_text += f"   👤 @{username} | {first_name}\n"
        users_text += f"   📥 {downloads} DL | 📅 {last_active}\n\n"
    
    users_text += f"\n📊 **Total Users:** {len(users_db)}"
    users_text += f"\n🚫 **Banned:** {sum(1 for u in users_db.values() if u.get('is_banned', False))}"
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    handlers = {
        ...
        "admin_users": lambda: show_users_list(update, context),
...
def main():
    ...
    # Start bot
    print("=" * 60)
    print("🤖 SYNAX DOWNLOAD BOT - ULTIMATE EDITION (ENHANCED & FIXED)")
    ...
    print("6. 🔑 Bulk Key Generation - Generate multiple keys at once")
